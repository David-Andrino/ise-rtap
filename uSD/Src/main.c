...
int main(void)
{
  ...
  while(1)
  {
    ...
    while ((counter) < ubNumberOfFiles)
    {
      ...
      /* Format the string */
      sprintf ((char*)str, "RTAP/%-11.11s", pDirectoryFiles[counter]);
			Storage_OpenReadFile(uwInternalBuffer, (const char*)str);
			counter++;

      if (Storage_CheckBitmapFile((const char*)str, &uwTxtlen) == 0)
      {
        /* Format the string */
        sprintf ((char*)str, "RTAP/%-11.11s", pDirectoryFiles[counter]);
        ...
        /* Open a file and copy its content to an internal buffer */
        Storage_OpenWriteFile(uwInternalBuffer, (const char*)str);
				Storage_OpenReadFile(uwInternalBuffer, (const char*)str);
